...
def get_fiscal_quarter_info(dt, is_nvda=False):
    ...
    if is_nvda:
        if month in [2, 3, 4]:
            ...
            q_num, fy = 4, (year + 1 if month in [11, 12] else year)
    else:
        q_num = (month - 1) // 3 + 1
        fy = year
...
def fetch_36_quarters_sec(ticker_symbol):
    ...
    try:
        ...
        tickers_data = cik_res.json()
        cik = None
        for entry in tickers_data.values():
            if entry.get('ticker', '').upper() == ticker_symbol.upper():
        ...
        for tag in ['Revenues', 'RevenueFromContractWithCustomerExcludingAssessedTax', 'SalesRevenueNet']:
            if tag in us_gaap:
                rev_units = us_gaap[tag].get('units', {}).get('USD', [])
                if rev_units:
                    break
        ...
        for tag in ['EarningsPerShareDiluted', 'EarningsPerShareBasicAndDiluted']:
            if tag in us_gaap:
                eps_units = us_gaap[tag].get('units', {}).get('USD/shares', [])
                if eps_units:
                    break
        ...
        eps_records = [
            {'Date': pd.to_datetime(item['end']), 'EPS': item['val'], 'form': item.get('form'), 'frame': item.get('frame', '')}
            for item in eps_units if item.get('form') in ['10-Q', '10-K'] and 'end' in item
        ]

        df_rev = pd.DataFrame(rev_records).drop_duplicates(subset=['Date', 'form', 'frame'])
        df_eps = pd.DataFrame(eps_records).drop_duplicates(subset=['Date', 'form', 'frame'])

        merged = pd.merge(df_rev, df_eps, on=['Date', 'form'], how='inner')
        merged = merged.sort_values('Date', ascending=True).reset_index(drop=True)
        
        return merged.tail(48).reset_index(drop=True)

    except Exception:
        return pd.DataFrame()

def process_36q_fundamentals(ticker_symbol, yf_q_financials=None):
    df_sec = fetch_36_quarters_sec(ticker_symbol)
    ...
    if not df_sec.empty and len(df_sec) >= 6:
        for _, row in df_sec.iterrows():
            dt = row['Date']
            q_str, fy, q_label = get_fiscal_quarter_info(dt, is_nvda=is_nvda)
            records.append({
                "Date": dt,
                "Q_Code": q_str,
                "FY": fy,
                "Quarter_Label": q_label,
                "Revenue": float(row['Revenue_x'] if 'Revenue_x' in row else row['Revenue']),
                "EPS": float(row['EPS_x'] if 'EPS_x' in row else row['EPS'])
            })
    elif yf_q_financials is not None and not yf_q_financials.empty:
        df_t = yf_q_financials.T.copy()
        df_t.index = pd.to_datetime(df_t.index)

        rev_cols = [c for c in df_t.columns if 'Total Revenue' in str(c) or (('Revenue' in str(c) or 'Operating Revenue' in str(c)) and 'Cost' not in str(c))]
        eps_cols = [c for c in df_t.columns if 'Diluted EPS' in str(c) or 'Basic EPS' in str(c) or 'Normalized EPS' in str(c)]
    ...
    summary = pd.DataFrame(records).drop_duplicates(subset=['Date']).sort_values('Date', ascending=True).reset_index(drop=True)
...
# ==========================================
# 5. CACHED MARKET DATA FETCHING ENGINE
# ==========================================
...
# ==========================================
# 6. TECHNICAL CALCULATIONS & PATTERN ENGINE
# ==========================================
...
if ticker_input and ticker_input != "N/A":
    ...
    if err or data is None:
        ...
        tab_tech, tab_fund, tab_peers = st.tabs([
            ...
            "📑 Extended 36-Quarter Fundamentals & Adjustments",
        ...
        with tab_fund:
            if not q_summary.empty:
                ...
                st.caption("Displays Quarterly Sales, YoY Revenue Growth, Quarterly EPS, QoQ & YoY EPS Growth, and Trailing Twelve Months (TTM) Totals derived from SEC filings (Most Recent at Top).")
                
                display_df = q_summary.copy()
                
                display_df['Quarterly Revenue'] = display_df['Quarterly Revenue ($)'].apply(format_large_number)
                display_df['Revenue Growth (YoY)'] = display_df['Revenue Growth (YoY)'].apply(lambda x: format_pct(x) if pd.notnull(x) else "—")
